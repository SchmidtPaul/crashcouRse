...
In diesem Beispiel werden wir ignorieren wie das Experiment eigentlich angelegt war (i.e. $\alpha$-lattice) und werten es stattdessen aus, als wäre es eine **vollständig randomisierte Anlage** gewesen. Demnach können wir die Spalten mit den Informationen zum eigentlich Versuchsdesign entfernen. Desweiteren nehmen wir an, dass nur 4 Sorten getestet wurden, damit Datensatz und Ergebnisse übersichtlicher sind.
...
ggplot(data=means$emmeans, aes(x=gen)) +
 ...
 labs(y="Adjustierter Ertragsmittelwert", x="Sorte", 
